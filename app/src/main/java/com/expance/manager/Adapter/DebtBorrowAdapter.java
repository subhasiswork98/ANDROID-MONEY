    ...
    public void onBindViewHolder(RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == 0) {
            ...
            if (((Debt) this.list.get(position + 1)).getStatus() == 0) {
                string = this.context.getResources().getString(R.string.not_receive_amount, beautifyAmount);
            } else {
                string = this.context.getResources().getString(R.string.receive_amount, beautifyAmount);
            }
        ...
        viewHolder.amountLabel.setTextColor(this.context.getResources().getColor(R.color.expense));
        ...
        if (debt.getStatus() == 0) {
            viewHolder.amountLabel.setText(Helper.getBeautifyAmount(str, debt.getAmount() - debt.getPay()));
            viewHolder.amountLabel.setPaintFlags(viewHolder.amountLabel.getPaintFlags() & (-17));
        } else {
            viewHolder.amountLabel.setText(Helper.getBeautifyAmount(str, debt.getAmount()));
